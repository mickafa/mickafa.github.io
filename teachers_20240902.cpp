/// версия от 02.09.2024, здесь по 8 уроков каждый день
...
int write_line ( int j, char *ll, char *rl, FILE *fc ){
    ...
    fprintf ( fc, ">%d", j + 5 ) ;            /// номер урока в правой клетке
    fprintf ( fc, "</td>\n" ) ;
...
int write_day ( const char* daily, int i, FILE* fc ){
    ...
    char *ls[8+1]; /// указатели на названия уроков
    ls[8] = &teachers[i-1]; /// 9-й урок всегда пустой

    for ( int j = 0; j < 8; j++ ){ /// находим названия всех уроков данного дня
        ls[j] = &teachers[i];
        while ( teachers[i] != ';' && teachers[i] != EOL ) i++;  /// ищем начало названия следующего урока
        teachers[i++] = '\0';
    }

    for ( int j = 0; j < 8; j++ ) x += strlen ( ls[j] );
    ...
    if ( x ){                   /// если есть хотя бы один урок в этот день
        ...
        for ( int j = 0; j < 4; j++ )        /// j = номер пары уроков: (1,5), (2,6), (3,7), (4,8)
